...
def do_work_gujarat(keyword):
	...
	filename='C:\\Sentiment-Analysis\\Nclfmodel.sav'
...
log = pd.read_csv("C:\\Sentiment-Analysis\\Login.csv")
...
def get_tweets(keyword):
	...
	for status in tweepy.Cursor(api.search,q=(query+" -filter:retweets")  and ("place:%s" %place_id),tweet_mode="extended",lang='en',locale='ja',result_type='recent',truncated=False).items():
    ...
    #posts = api.user_timeline(screen_name=keyword, count = 199, lang ="en", tweet_mode="extended")
...
def bar_plot(data1,keyword1):	
	...
	plt.savefig('C:\\Sentiment-Analysis\\static\\img\\'+keyword1+'-bar.png')
	plt.clf() 
	plt.cla()
	plt.close()
...
def pie_plot(keys,x1,x2):
	...
	plt.savefig('C:\\Sentiment-Analysis\\static\\img\\'+temp+'.png')
	plt.clf() 
	plt.cla()
	plt.close()
